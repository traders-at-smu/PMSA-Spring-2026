...
def _consolidate_soccer_trio(results: list[dict]) -> list[dict]:
    ...
    for (slug, event_ticker), recs in groups.items():
        ...
        if len(recs) == 2:
            # Binary game (no draw) — consolidate into one row with kalshi_t2_ticker
            draw_recs = [r for r in recs
                         if r.get("poly_outcome", "").strip().lower() in ("draw", "tie", "drawn")]
            if draw_recs:
                final.extend(recs)
                continue
            # Determine T1/T2 order from first rec's outcomes list
            try:
                all_outcomes = json.loads(recs[0].get("poly_outcomes", "[]"))
            except Exception:
                all_outcomes = []
            if len(all_outcomes) >= 2:
                order = {o.strip().lower(): i for i, o in enumerate(all_outcomes)}
                recs.sort(key=lambda r: order.get(r.get("poly_outcome", "").strip().lower(), 99))
            t1_rec, t2_rec = recs[0], recs[1]
