...
# Output goes directly into V6.3/input_files/ so the bot can pick it up.
OUTPUT_DIR = os.path.join(BASE_DIR, "..", "..", "V6.3", "input_files")

def get_output_filename():
    """Generates a filename in the format output-mm-dd-yy.csv."""
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    now = datetime.now()
    return os.path.join(OUTPUT_DIR, now.strftime("output-%m-%d-%y.csv"))
