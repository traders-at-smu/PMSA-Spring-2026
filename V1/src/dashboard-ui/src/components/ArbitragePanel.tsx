...
export function ArbitragePanel({ paused }: { paused: boolean }) {
  const { data, connected } = useSSE<ScreenerResults>("/api/screener/stream", paused);
  ...
  return (
    <div className="space-y-4">
      ...
      {!screener ? (
        <div className="text-center text-zinc-500 py-12">Loading screener data...</div>
      ) : (
        <>
          {/* ---- Top Spreads ---- */}
          {section === "spreads" && (
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
              {screener.topSpreads.length === 0 ? (
                ...
                screener.topSpreads.map((s) => (
                  ...
                  >
                    <div className="text-sm text-zinc-200 font-medium mb-3 line-clamp-2">
                      {s.market}
                    </div>
                    ...
                    <div className="flex justify-between mt-3 pt-3 border-t border-zinc-800 text-xs text-zinc-500">
                      ...
                      <span>
                        Vol 24h: <span className="text-zinc-400">{formatUsd(s.volume24hr)}</span>
                      </span>
                    </div>

                    {(s.bidDepth || s.askDepth) && (
                      <div className="flex justify-between mt-1 text-xs text-zinc-600">
                        <span>Bid depth: {s.bidDepth ? formatUsd(s.bidDepth) : "—"}</span>
                        <span>Ask depth: {s.askDepth ? formatUsd(s.askDepth) : "—"}</span>
                      </div>
                    )}
                  </div>
                ))
                  ...
                  >
                    ...
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <div className="text-sm text-zinc-200 font-medium">{arb.event}</div>
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="text-zinc-500 text-xs uppercase tracking-wider">
                              ...
                              <th className="pb-2 text-right">Best Bid</th>
                              <th className="pb-2 text-right">Best Ask</th>
