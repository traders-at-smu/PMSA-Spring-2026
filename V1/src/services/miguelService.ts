...
export class MiguelService {
  ...
  private getPaths() {
    ...
    const scriptsDir = path.resolve(process.cwd(), settings.python.miguelScriptsDir);
    return {
      pythonExecutable: settings.python.pythonExecutable,
      pollIntervalSec: settings.python.miguel.pollIntervalSec,
      minPairs: settings.python.miguel.minPairs,
      scriptsDir,
