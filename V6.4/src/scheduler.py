...
def run_generator(generator_dir):
    ...
    print(f"  {Fore.CYAN}│{Style.RESET_ALL}  {Style.BRIGHT}SCHEDULED UPDATE:{Style.RESET_ALL} Running Pairs Generator V6.4...  {Fore.CYAN}│{Style.RESET_ALL}")
    print(f"  {Fore.CYAN}└──────────────────────────────────────────────────────────┘{Style.RESET_ALL}")
    
    try:
        import sys
        # Run main.py in Pairs Generator V6.4
        result = subprocess.run([sys.executable, "main.py"], cwd=generator_dir, check=True, capture_output=True)
        print(f"  {Style.DIM}[{ts}] [scheduler] Pairs Generator V6.4 finished successfully.{Style.RESET_ALL}")
        return True
    except subprocess.CalledProcessError as e:
        print(f"  {Fore.RED}[{ts}] [scheduler] ERROR running Pairs Generator V6.4: {e}{Style.RESET_ALL}")
        if e.stderr:
...
def scheduler_loop(cfg):
    ...
    generator_dir = root_dir / "Pairs Generator V6.4"
