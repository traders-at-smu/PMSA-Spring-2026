...
def _resolve_execution_mode(cfg: dict[str, Any]) -> str:
    mode = str(cfg.get("mode", "live")).lower()
    if mode != "live":
        raise RuntimeError(
            f"VE6.2 is live-only — set mode='live' in config.json (got '{mode}')"
        )
    if not _live_creds_present(cfg):
        raise RuntimeError(
            "Live credentials missing — set kalshi and polymarket keys in api-keys.json"
        )
    return "live"
...
def execute_live(opp: dict[str, Any], kalshi, poly, log_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        # Only play sound if fully filled
...
def run_loop(
    ...
    while True:
        try:
            ...
            # Round-robin rotation — all pairs treated equally regardless of position state
            rotated = scannable[pair_offset:] + scannable[:pair_offset]
            scan_pairs = rotated[:pairs_per_cycle]
            pair_offset = (pair_offset + len(scan_pairs)) % max(len(scannable), 1)
