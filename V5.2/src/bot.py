...
def _subtract_consumed_bids(
    real_levels: list[dict[str, Any]],
    consumed: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Subtract previously consumed paper bids from real orderbook levels.

    For each consumed entry, reduce the matching real level's size. If the
    real book grew since we last checked, the new liquidity shows through.
    """
    consumed_map: dict[float, int] = {}
    for c in consumed:
        p = round(float(c["price"]), 6)
        consumed_map[p] = consumed_map.get(p, 0) + int(c["size"])

    adjusted: list[dict[str, Any]] = []
    for level in real_levels:
        p = round(float(level["price"]), 6)
        real_size = int(level["size"])
        used = consumed_map.get(p, 0)
        remaining = max(0, real_size - used)
        if remaining > 0:
            adjusted.append({"price": level["price"], "size": remaining})
    return adjusted


def _merge_consumed_bids(
    existing: list[dict[str, Any]],
    new_fills: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Merge newly consumed bids into the existing consumed list, grouped by price."""
    by_price: dict[float, int] = {}
    for c in existing:
        p = round(float(c["price"]), 6)
        by_price[p] = by_price.get(p, 0) + int(c["size"])
    for c in new_fills:
        p = round(float(c["price"]), 6)
        by_price[p] = by_price.get(p, 0) + int(c["size"])
    return [{"price": p, "size": s} for p, s in sorted(by_price.items(), reverse=True)]
...
def _process_exit_positions(
    ...
    for pair_id, position in list(positions.items()):
        ...
        # Paper mode: subtract previously consumed bids so the bot doesn't
        # sell into the same liquidity over and over
        if mode == "paper":
            k_levels = _subtract_consumed_bids(
                k_levels, position.get("consumed_k_bids", []),
            )
            p_levels = _subtract_consumed_bids(
                p_levels, position.get("consumed_p_bids", []),
            )
        ...
        # Paper mode: track which bids were consumed by this exit so the next
        # exit check doesn't sell into the same liquidity.
        # walk["slippage"] stores CUMULATIVE contracts, so we diff consecutive
        # entries to get the per-level size that was actually taken.
        if mode == "paper" and exit_contracts > 0:
            exit_slippage = walk.get("slippage", [])
            new_k_consumed: list[dict[str, Any]] = []
            new_p_consumed: list[dict[str, Any]] = []
            prev_cum = 0
            for s in exit_slippage:
                cum = int(s.get("contracts", 0))
                delta = cum - prev_cum
                prev_cum = cum
                if delta <= 0:
                    continue
                new_k_consumed.append({"price": s["k_price"], "size": delta})
                new_p_consumed.append({"price": s["p_price"], "size": delta})
            positions[pair_id]["consumed_k_bids"] = _merge_consumed_bids(
                position.get("consumed_k_bids", []), new_k_consumed,
            )
            positions[pair_id]["consumed_p_bids"] = _merge_consumed_bids(
                position.get("consumed_p_bids", []), new_p_consumed,
            )
