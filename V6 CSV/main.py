...
def _print_banner(cfg: dict, pairs_count: int) -> None:
    ...
    log_path = cfg.get("entry_log", "data/entry_trades.json")
    interval = cfg.get("scan_interval_seconds", 5)

    label_w = 12
    print(f"\n  {Style.BRIGHT}Kalshi × Polymarket Arb Bot{Style.RESET_ALL}  v6.0")
    print(f"  {'─' * 42}")
...
def cmd_scan(args) -> int:
    ...
    # Override pairs_file with the latest CSV from input_files if configured
